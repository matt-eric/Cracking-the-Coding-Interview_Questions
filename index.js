...
(() => {
    ...
    const getCurrentQuestionObject = ( displayName ) => {
        return chapters.all.find(c => c.questions.find(q => q.displayName === displayName)).questions.find(q => q.displayName === displayName);
    };
    ...
    for (let i=0; i<chapters.all.length; i++) {
        const label = document.createElement('option');
        label.disabled = true;
        label.innerText = `---${chapters.all[i].displayName}---`;
        select.appendChild(label);
        for (let c = 0; c < chapters.all[i].questions.length; c++) {
            const name = chapters.all[i].questions[c].displayName;
